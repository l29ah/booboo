...
EHG_REPO_URI="https://dev.mutt.org/hg/mutt"
patchset_base=1.6.1
patchset=mutt-${patchset_base}-gentoo-patches${PATCHSET_REV}
SRC_URI="https://dev.gentoo.org/~grobian/distfiles/$patchset.tar.xz"
IUSE="berkdb crypt debug doc gdbm gnutls gpg idn imap kerberos libressl mbox nls nntp pop qdbm sasl selinux sidebar slang smime smtp ssl tokyocabinet +vanilla"
SLOT="0"
LICENSE="GPL-2"
KEYWORDS=""
CDEPEND="
...
src_prepare() {
	...
	use vanilla || for p in "${PATCHDIR}"/[0-9][0-9]-*.patch ; do
		epatch "${p}"
	done
	...
	sed -i -e 's/"Mutt %s (%s)"/"Mutt %s (%s, Gentoo booboo '"${PVR}${upatches}"')"/' \
