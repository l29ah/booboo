...
# $Header: /var/cvsroot/gentoo-x86/games-strategy/freeciv/freeciv-2.4.4.ebuild,v 1.2 2015/03/09 11:44:46 ago Exp $
...
IUSE="auth aimodules dedicated +gtk gtk3 ipv6 mapimg modpack mysql nls postgres qt5 readline sdl +server +sound sqlite system-lua"
...
RDEPEND="app-arch/bzip2
	...
	!dedicated? (
		...
		!sdl? ( !gtk? ( !gtk3? ( !qt5? ( x11-libs/gtk+:2 ) ) ) )
	)"
DEPEND="${RDEPEND}
	virtual/pkgconfig
	!dedicated? (
...
src_configure() {
	local myclient myopts mydatabase mymodpack
	...
	if use dedicated ; then
		...
		if use !sdl && use !gtk && use !gtk3 && use !qt; then
			...
			use gtk3 && myclient="${myclient} gtk3" && mymodpack="gtk3"
	...
	egamesconf \
		...
		--enable-fcmp="$(usex modpack "${mymodpack}" "no")" \
