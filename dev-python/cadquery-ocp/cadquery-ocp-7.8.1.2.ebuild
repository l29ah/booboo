...
# keep in sync with dev-python/clang dependency
LLVM_COMPAT=( {17..19} )
...
# FIXME https://github.com/CadQuery/OCP/issues/132
...
SRC_URI="https://github.com/CadQuery/OCP/archive/refs/tags/${MY_PV}.tar.gz -> ${P}.tar.gz
	https://github.com/CadQuery/OCP/releases/download/${MY_PV}/${PREBUILT_STUBS_NAME}.zip -> ${P}-${PREBUILT_STUBS_NAME}.zip"
S="${WORKDIR}/${MY_P}"
...
RDEPEND="
	${PYTHON_DEPS}
	sci-libs/opencascade:0/7.8[json,vtk]
...
src_prepare() {
	...
	local _CLANG_VERSION="$(CC=clang CPP=clang clang-fullversion)"
	...
	local _CLANG_INCLUDE_DIR="${EPREFIX}/usr/lib/clang/$(CC=clang CPP=clang clang-major-version)/include"
	...
	mv "${WORKDIR}/OCP_src_stubs_ubuntu-20.04" OCP || die
...
src_configure() {
	local mycmakeargs=(
		...
		#-D3RDPARTY_TBB_LIBRARY_DIR=/usr/include/tbb
...
src_install() {
	python_moduleinto .
	python_domodule "${S}/OCP.build/OCP/"OCP*.so
}
